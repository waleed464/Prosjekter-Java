...
class Labyrint{
    protected Rute[][] rute;
    protected Lenkeliste<Lenkeliste<Rute>> mulige_ruter;
    public Monitor alle_mulige_ruter = new Monitor();
    protected int rader;
...
*/
    ...
    // Denne metoden skriver ut info uten detaljer i terminalen. Altsaa vi ikke taster inn ordet "detaljert" naar vi kjoerer programmet
    // i terminalen. Da havner vi altsaa inn i metoden under:
    ...
    // Denne metoden skriver ut info med detaljer i terminalen. Altsaa vi taster inn ordet "detaljert" etter
    // hvordan det er forklart i oppgaven naar vi kjoerer programmet i terminalen. Da havner vi altsaa inn i metoden under:
    ...
    public void finnUtveiFra(int rad, int kol) {
        try{
            Lenkeliste<Rute> vei = new Lenkeliste<>();
    		Lenkeliste<Rute> start = new Lenkeliste<>();
    		start.leggTil(rute[rad][kol]);

            while(alle_mulige_ruter.hentListe().stoerrelse()!=0){
                alle_mulige_ruter.hentListe().fjern();
            }

    		//Runnable task = new MyThread(start, vei); //Her er en oppgave
            Runnable hovedOppg=new MinTraad(rute[rad][kol], vei);
            Thread hovedTraad = new Thread(hovedOppg); //Her er en arbeider
    		hovedTraad.start(); //Leggo
    		...
    		Lenkeliste<Lenkeliste<Rute>> r = alle_mulige_ruter.hentListe();
            skrivut("Antall utveier: "+Integer.toString(r.stoerrelse()));
    		for(Lenkeliste<Rute> v : r){
                //System.out.println(v.toString());
                for(Rute elem:v){
                    System.out.print(elem.koordTilStreng()+"--> ");
                }
